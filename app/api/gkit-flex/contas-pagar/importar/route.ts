...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(file instanceof File)) {
      return Response.json({ error: 'Envie a planilha de contas a pagar.' }, { status: 400 });
    }

    const parsedRows = await parsePayablesWorkbook(file);
    const result = await importPayables(competencia, parsedRows, file.name);
    return Response.json({ ...result, imported: parsedRows.length });
  } catch (error) {
    console.error('[contas-pagar/importar][POST]', error);
    return Response.json({ error: error instanceof Error ? error.message : 'Erro ao importar contas a pagar.' }, { status: 500 });
  }
}
