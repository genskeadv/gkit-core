-- Allow catch-up runs to select only processes that were not synced after a
-- given cutoff, while keeping the regular automatic queue behavior unchanged.

drop function if exists gkit_jur.proximos_processos_sync(integer, text, uuid, uuid[]);
  ...
  with base as (
    ...
    where p.status = 'ativo'
      ...
      and (
        ...
        or p.ultima_sincronizacao_em is null
        or p.ultima_sincronizacao_em < p_synced_before
      )
