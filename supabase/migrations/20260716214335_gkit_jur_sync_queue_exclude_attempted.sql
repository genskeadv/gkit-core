-- Avoid retrying the same DataJud candidates across batches in a single sync run.
-- The exclusion list is supplied by the application runner and is intentionally
-- ignored for targeted single-process syncs.

drop function if exists gkit_jur.proximos_processos_sync(integer, text, uuid);
...
revoke all on function gkit_jur.proximos_processos_sync(integer, text, uuid, uuid[]) from public, anon, authenticated;
grant execute on function gkit_jur.proximos_processos_sync(integer, text, uuid, uuid[]) to service_role;
