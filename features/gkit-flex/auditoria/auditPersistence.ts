...
export async function getMonthlyAudit(competenciaInput?: string | null) {
  ...
  if (pmError) throw new Error(`Erro ao consultar competência de contas a pagar: ${pmError.message}`);
  ...
  if (execError) throw new Error(`Erro ao consultar versões de comissão: ${execError.message}`);
  ...
  if (summaryResult.error) throw new Error(`Erro ao consultar resumos de comissão: ${summaryResult.error.message}`);
  if (auditResult.error) throw new Error(`Erro ao consultar auditoria de comissão: ${auditResult.error.message}`);
  ...
  const payableRows = payableMonth?.id
    ...
    : { data: [], error: null };
  if (payableRows.error) throw new Error(`Erro ao consultar contas a pagar: ${payableRows.error.message}`);
  ...
  const importacoes = payableMonth?.id
    ? await supabase
        ...
        .limit(20)
    : { data: [], error: null };
  if (importacoes.error) throw new Error(`Erro ao consultar importações: ${importacoes.error.message}`);
  ...
  const checklist: ChecklistItem[] = [
    ...
    },
    {
      ...
      detalhe: auditRows.length || semVendedor ? `${auditRows.length} apontamento(s) e ${semVendedor} lançamento(s) sem vendedor.` : 'Sem apontamentos relevantes.',
    },
    {
      id: 'contas_pagar_mes_aberto_ou_fechado',
      titulo: 'Competência de contas a pagar existe',
      status: payableMonth ? 'ok' : 'bloqueio',
      detalhe: payableMonth ? `Status: ${payableMonth.status}` : 'Abra o mês antes de importar ou fechar contas a pagar.',
    },
    {
      id: 'contas_pagar_importadas',
      titulo: 'Contas a pagar importadas/cadastradas',
      status: payables.length ? 'ok' : 'bloqueio',
      detalhe: payables.length ? `${payables.length} item(ns), total ${payableTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.` : 'Nenhuma conta a pagar cadastrada para o mês.',
    },
    {
      id: 'qualidade_contas_pagar',
      titulo: 'Qualidade do contas a pagar',
      status: semCategoria || valorZerado || vencimentoInvalido ? 'aviso' : 'ok',
      detalhe: `${semCategoria} sem categoria, ${valorZerado} com valor zerado/negativo, ${vencimentoInvalido} sem vencimento válido.`,
    },
    {
      id: 'comissoes_no_pagar',
      titulo: 'Comissões sincronizadas no contas a pagar',
      status: latestExecution && Math.abs(commissionDifference) > 0.02 ? 'aviso' : 'ok',
      detalhe: latestExecution
        ? `Comissão calculada: ${expectedCommission.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}; no contas a pagar: ${commissionPayablesTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}.`
        : 'Sem cálculo de comissão para comparar.',
    },
  ];
  ...
  return {
    ...
    auditoria: {
      ...
      porProblema: countBy(auditRows as any[], 'problema', 'Sem descrição'),
...
export async function buildMonthlyClosingWorkbook(competenciaInput?: string | null): Promise<{ competencia: string; filename: string; buffer: Buffer }> {
  ...
  const resumo = [
    ...
    { Indicador: 'Total contas a pagar', Valor: data.contasPagar?.total || 0 },
